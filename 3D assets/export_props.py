# Export finger + gun props to game-space OBJ + MTL.
#   /Applications/Blender.app/Contents/MacOS/Blender --background \
#       --python "3D assets/export_props.py"
# Source: 3D assets/<name>.blend
# Output: app/src/main/assets/models/<name>.obj (+ .mtl)
import bpy, os
...
for name in PROPS:
    ...
    if not os.path.exists(src):
        print(f"!! {name}: {src} missing, skipped"); continue
    ...
    for nm in [o.name for o in bpy.data.objects]:
        ...
        try: ob.select_set(True); active = ob
        except Exception: pass
    if active is None:
