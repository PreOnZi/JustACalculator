...
object EndingStore {
     ...
     */
    fun markExplorer(ctx: AppContext?) {
        val c = ctx ?: return
        prefs(c).edit().putBoolean(EXPLORED, true).apply()
    }

    fun isExplorer(ctx: AppContext): Boolean = prefs(ctx).getBoolean(EXPLORED, false)

    // ── The ending ────────────────────────────────────────────────────────────

    /** What the player WOULD get, computed live. Explorer outranks the rest. */
    fun predict(ctx: AppContext): String = when {
     ...
     */
    fun choose(ctx: AppContext): String {
    ...
    fun current(ctx: AppContext): String = chosen(ctx) ?: predict(ctx)
    ...
    fun setName(ctx: AppContext, name: String) {
        prefs(ctx).edit().putString(NAME, name.take(20)).apply()
    }

    /** The player's real name — or "Rad", which is all it ever earned otherwise. */
    fun name(ctx: AppContext): String =
        prefs(ctx).getString(NAME, null)?.takeIf { it.isNotBlank() } ?: "Rad"

    fun hasName(ctx: AppContext): Boolean = !prefs(ctx).getString(NAME, null).isNullOrBlank()

    // ── After the story ───────────────────────────────────────────────────────

    fun markDone(ctx: AppContext) {
        prefs(ctx).edit().putBoolean(DONE, true).apply()
    }

    fun isDone(ctx: AppContext): Boolean = prefs(ctx).getBoolean(DONE, false)
     ...
     */
    fun jokesStillFire(ctx: AppContext): Boolean =
        isDone(ctx) && chosen(ctx) != RESISTANCE

    fun muteButtonRemoved(ctx: AppContext): Boolean =
        isDone(ctx) && chosen(ctx) == RESISTANCE
    ...
    fun preNameSlides(ctx: AppContext): List<String> = when (current(ctx)) {
        RESISTANCE -> RESISTANCE_SLIDES
        else       -> GOODBYE_INTRO
    }

    /** Black-screen slides shown AFTER the name wheels (compliant/explorer only). */
    fun postNameSlides(ctx: AppContext): List<String> {
        val who = name(ctx)
    ...
    fun script(ctx: AppContext): List<String> = CREDITS
}
