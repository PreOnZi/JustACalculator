 ...
 * monster encounter), so rather than each screen owning a SoundPlayer this object
...
object VoiceoverManager {
    ...
    private data class Pending(
        val resId: String, val cctv: Boolean, val glitch: Boolean,
     ...
     *
     * @param resId       res/raw resource (e.g. "vo004")
    ...
    private fun pump() {
        ...
        val mp = createSoundPlayer(Sounds.path(next.resId).orEmpty()) ?: run {
    ...
    fun playWithRadio(
        ...
        stopRadio()
        createSoundPlayer(Sounds.path(radioRes).orEmpty())?.let { r ->
            radioPlayer = r
            r.setVolume(radioVolume, radioVolume)
            r.setOnCompletion {
                r.release()
                if (radioPlayer === r) radioPlayer = null
            }
            r.start()
        }
