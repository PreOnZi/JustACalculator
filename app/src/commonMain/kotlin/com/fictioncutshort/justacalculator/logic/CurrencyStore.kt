 ...
 * "calc_city" Prefs store — the same one the rest of the city uses.
 *
...
object CurrencyStore {
    ...
    fun balance(ctx: AppContext, c: Currency): Int = prefs(ctx).getInt(c.prefKey, 0)
     ...
     */
    fun award(ctx: AppContext, c: Currency, amount: Int, sourceTag: String) {
        val p = prefs(ctx)
    ...
    fun nonZero(ctx: AppContext): List<Pair<Currency, Int>> =
        Currency.entries
            .sortedBy { it.order }
    ...
    fun setBalance(ctx: AppContext, c: Currency, value: Int) {
    ...
    fun spend(ctx: AppContext, c: Currency, amount: Int): Int {
    ...
    fun zero(ctx: AppContext, c: Currency): Int {
    ...
    fun stakeCoins(ctx: AppContext, numbers: List<Int>) {
        prefs(ctx).edit()
    ...
    fun lotteryStaked(ctx: AppContext): Boolean = prefs(ctx).getBoolean(LOTTO_STAKED, false)
    fun lotterySettled(ctx: AppContext): Boolean = prefs(ctx).getBoolean(LOTTO_SETTLED, false)

    /** Whether the city "draw closing soon" popup has already been shown once. */
    fun lotteryShown(ctx: AppContext): Boolean = prefs(ctx).getBoolean(LOTTO_SHOWN, false)
    fun markLotteryShown(ctx: AppContext) {
        prefs(ctx).edit().putBoolean(LOTTO_SHOWN, true).apply()
    }

    /** The six numbers the player entered (empty if never staked). */
    fun lotteryNumbers(ctx: AppContext): List<Int> =
    ...
    fun settleLottery(ctx: AppContext): Int {
    ...
    fun gamesPlayed(ctx: AppContext): Set<String> =
        prefs(ctx).getString(GAMES_PLAYED, "").orEmpty()
            .split(",").filter { it.isNotBlank() }.toSet()

    fun gamePlayed(ctx: AppContext, id: String): Boolean = id in gamesPlayed(ctx)

    fun markGamePlayed(ctx: AppContext, id: String) {
    ...
    fun arcadeGamesDone(ctx: AppContext): Boolean =
        ARCADE_GAME_IDS.all { gamePlayed(ctx, it) }

    fun building8Complete(ctx: AppContext): Boolean =
