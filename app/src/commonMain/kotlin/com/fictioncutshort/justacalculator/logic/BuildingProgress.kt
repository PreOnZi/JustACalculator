...
object BuildingProgress {
    ...
    fun putInt(ctx: AppContext, digit: Int, key: String, v: Int) =
        prefs(ctx).edit().putInt(k(digit, key), v).apply()

    fun getInt(ctx: AppContext, digit: Int, key: String, def: Int = 0): Int =
        prefs(ctx).getInt(k(digit, key), def)

    fun putFloat(ctx: AppContext, digit: Int, key: String, v: Float) =
        prefs(ctx).edit().putFloat(k(digit, key), v).apply()

    fun getFloat(ctx: AppContext, digit: Int, key: String, def: Float = 0f): Float =
        prefs(ctx).getFloat(k(digit, key), def)

    fun putBool(ctx: AppContext, digit: Int, key: String, v: Boolean) =
        prefs(ctx).edit().putBoolean(k(digit, key), v).apply()

    fun getBool(ctx: AppContext, digit: Int, key: String, def: Boolean = false): Boolean =
        prefs(ctx).getBoolean(k(digit, key), def)

    fun putString(ctx: AppContext, digit: Int, key: String, v: String) =
        prefs(ctx).edit().putString(k(digit, key), v).apply()

    fun getString(ctx: AppContext, digit: Int, key: String, def: String = ""): String =
        prefs(ctx).getString(k(digit, key), def) ?: def

    /** A set of names — which apps were opened, which exhibits were read. */
    fun putSet(ctx: AppContext, digit: Int, key: String, v: Set<String>) =
        prefs(ctx).edit().putStringSet(k(digit, key), v).apply()

    fun getSet(ctx: AppContext, digit: Int, key: String): Set<String> =
        prefs(ctx).getStringSet(k(digit, key), emptySet()) ?: emptySet()

    /** A list of numbers — collected key ids, visited checkpoints, tile colours. */
    fun putInts(ctx: AppContext, digit: Int, key: String, v: List<Int>) =
        putString(ctx, digit, key, v.joinToString(","))

    fun getInts(ctx: AppContext, digit: Int, key: String): List<Int> =
        getString(ctx, digit, key)
            .split(",")
            .mapNotNull { it.trim().toIntOrNull() }

    fun putFloats(ctx: AppContext, digit: Int, key: String, v: List<Float>) =
        putString(ctx, digit, key, v.joinToString(","))

    fun getFloats(ctx: AppContext, digit: Int, key: String): List<Float> =
        getString(ctx, digit, key)
            .split(",")
            .mapNotNull { it.trim().toFloatOrNull() }

    /** True once this building has written anything — i.e. there's a run to resume. */
    fun hasSave(ctx: AppContext, digit: Int): Boolean =
        prefs(ctx).getAll().keys.any { it.startsWith("bp_${digit}_") }

    /**
     * Wipe a building's saved run. Called on completion: the next visit should be
     * a fresh start, not a resume into a finished game.
     */
    fun clear(ctx: AppContext, digit: Int) {
        val p = prefs(ctx)
        val e = p.edit()
        for (key in p.getAll().keys) if (key.startsWith("bp_${digit}_")) e.remove(key)
        e.apply()
    }
}
