...
object ComplicityStore {
     ...
     */
    fun recordConversationChoice(ctx: AppContext?, step: Int, accepted: Boolean) {
        val c = ctx ?: return
    ...
    private fun answered(ctx: AppContext): List<Pair<Int, Boolean>> {
        val p = prefs(ctx)
    ...
    fun wentAlongCount(ctx: AppContext): Int = answered(ctx).count { it.second }
    fun pushedBackCount(ctx: AppContext): Int = answered(ctx).count { !it.second }
     ...
     */
    fun phase1Complicity(ctx: AppContext): Float {
    ...
    fun recordBuilding6(ctx: AppContext, paidToSolo: Boolean) {
        prefs(ctx).edit()
    ...
    fun paidToSolo(ctx: AppContext): Boolean = prefs(ctx).getBoolean(P2_PAID_TO_SOLO, false)
    fun building6Answered(ctx: AppContext): Boolean = prefs(ctx).getBoolean(P2_B6_ANSWERED, false)

    fun recordRating(ctx: AppContext, rating: Int) {
        prefs(ctx).edit()
    ...
    fun rating(ctx: AppContext): Int = prefs(ctx).getInt(P2_RATING, -1)
    fun rated(ctx: AppContext): Boolean = prefs(ctx).getBoolean(P2_RATED, false)
     ...
     */
    fun score(ctx: AppContext): Float {
    ...
    fun isComplicitEnding(ctx: AppContext): Boolean = score(ctx) >= 0.5f

    /**
     * Debug only: slam every input to one end so an ending can be tested without
     * replaying the whole story.
     */
    fun forceEnding(ctx: AppContext, complicit: Boolean) {
    ...
    fun summary(ctx: AppContext): String {
        ...
        return "p1 ${wentAlongCount(ctx)} along / ${pushedBackCount(ctx)} resisted " +
            ...
            "=> ${formatFixed(score(ctx).toDouble(), 2)} ${if (isComplicitEnding(ctx)) "COMPLICIT" else "REFUSED"}"
    }
}
