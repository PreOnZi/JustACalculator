...
object CalculatorActions {
    ...
    private fun loadTermsAccepted(): Boolean {
        ...
        logDebug("JustACalc", "loadTermsAccepted: $result, prefs null? ${prefs == null}")
        return result
    }
    ...
    fun persistTermsAccepted() {
        ...
        logDebug("JustACalc", "persistTermsAccepted called, prefs null? ${prefs == null}")
    }
    ...
    fun init(context: AppContext) {
        ...
        prefs = context?.openPrefs(PREFS_NAME)
        logDebug("JustACalc", "init called, prefs initialized: ${prefs != null}, termsAccepted: ${loadTermsAccepted()}")
    }
    ...
    fun persistConversationStep(step: Int) {
        ...
        if (step == 0) {
            ...
            logDebug("JustACalc", "persistConversationStep: $step")
        }
    ...
    fun persistInConversation(inConvo: Boolean) {
        if (!inConvo) {
            logWarn("JustACalc", "🚨 persistInConversation(FALSE) — stack trace:\n" + Throwable().stackTraceToString())
        } else {
            logDebug("JustACalc", "persistInConversation: $inConvo")
        }
    ...
    fun loadInitialState(): CalculatorState {
        ...
        logDebug("JustACalc", "prefs null? ${prefs == null}")
        logDebug("JustACalc", "savedStepRaw: $savedStepRaw")
        logDebug("JustACalc", "savedInConvo: $savedInConvo")
        logDebug("JustACalc", "savedCount (equalsCount): $savedCount")
        logDebug("JustACalc", "savedMuted: $savedMuted")
        logDebug("JustACalc", "savedPausedAtStep: $savedPausedAtStep")
        logDebug("JustACalc", "ALL PREFS: ${prefs?.getAll()}")
        ...
        val savedStep = if (savedStepRaw == 0 && savedPausedAtStep > 0) {
            logWarn("JustACalc", "CORRUPT-STATE RECOVERY: savedStep=0 but savedPausedAtStep=$savedPausedAtStep — recovering to paused step.")
        ...
        logDebug("JustACalc", "getSafeStep($savedStep) returned: ${getSafeStep(savedStep)}")
        logDebug("JustACalc", "actualStep will be: $actualStep")
        logDebug("JustACalc", "==========================================")
        ...
        logDebug("JustACalc", "shouldShowMessage: $shouldShowMessage (inConvo=$effectiveInConvo, count=$savedCount, muted=$savedMuted)")
        val stepConfig = getStepConfig(actualStep)
        logDebug("JustACalc", "stepConfig.promptMessage: '${stepConfig.promptMessage.take(50)}'")
        logDebug("JustACalc", "========== END loadInitialState ==========")
    ...
    fun toggleConversation(state: MutableState<CalculatorState>) {
        ...
        logDebug("JustACalc", "toggleConversation: newMuted=$newMuted, step=${current.conversationStep}, " +
    ...
    fun resetGame(state: MutableState<CalculatorState>) {
        ...
        appContext?.openPrefs("calc_city")
            ?.edit()?.clear()?.apply()
    ...
    fun checkCameraMidpoint(state: MutableState<CalculatorState>): Boolean {
        ...
        if (current.cameraActive && current.cameraTimerStart > 0 && !current.cameraHasSwitched) {
            val elapsed = nowMillis() - current.cameraTimerStart
    ...
    fun handleInput(state: MutableState<CalculatorState>, action: String) {
        val current = state.value
        if (action == "+" || action == "-") {
            logDebug("JustACalc", "handleInput: action=$action, step=${current.conversationStep}, " +
...
// Block all input during rant mode
        ...
        if (current.showBrowser || (current.browserPhase > 0 && current.conversationStep !in listOf(111, 112, 113, 114)))  {
            logDebug("JustACalc", "!! INPUT BLOCKED by browser: showBrowser=${current.showBrowser}, browserPhase=${current.browserPhase}, step=${current.conversationStep}")
            return
        }
        ...
        if (current.silentUntil > 0 && nowMillis() < current.silentUntil) {
            // Just do calculator operations silently
            handleCalculatorInput(state, action)
            return
        } else if (current.silentUntil > 0 && nowMillis() >= current.silentUntil) {
        ...
        // Check if in timeout
        if (current.timeoutUntil > 0 && nowMillis() < current.timeoutUntil) {
            if (action == "=") {
                val now = nowMillis()
                if (lastOp == "=" && (now - lastOpTimeMillis) <= DOUBLE_PRESS_WINDOW_MS) {
                    if (nowMillis() >= current.timeoutUntil) {
        ...
        if (current.inConversation) {
            ...
            when (action) {
                "+" -> {
                    if (lastOp == "+" && (now - lastOpTimeMillis) <= DOUBLE_PRESS_WINDOW_MS) {
                        ...
                        logDebug("JustACalc", "++ DETECTED at step ${confirmState.conversationStep}, " +
                        ...
                        logDebug("JustACalc", "+ single tap (applied as operator), step=${current.conversationStep}, inConvo=${current.inConversation}")
            ...
            if (action == "+") {
                logDebug("JustACalc", "!! + pressed but inConversation=FALSE, step=${current.conversationStep}, isMuted=${current.isMuted}")
            }
...
// =====================================================================
    ...
    private fun formatPausedDouble(result: Double): String {
        ...
        return if (result == result.toLong().toDouble() &&
            ...
            val formatted = formatFixed(result, 10)
    ...
    private fun formatResult(result: String): String {
        ...
        return if (d == d.toLong().toDouble()) {
            ...
            formatFixed(d, 10).trimEnd('0').trimEnd('.')
        }
    }
    ...
    private fun handleNumberConfirmation(state: MutableState<CalculatorState>) {
        ...
        if (stepConfig.ageBasedBranching) {
            ...
            if (age != null) {
                ...
                val timeoutUntil = nowMillis() + (stepConfig.timeoutMinutes * 60 * 1000)
                state.value = current.copy(
                    number1 = "0",
                    number2 = "",
                    operation = null,
    ...
    private fun handleConversationResponse(state: MutableState<CalculatorState>, accepted: Boolean) {
        ...
        logDebug("JustACalc", "handleConversationResponse: accepted=$accepted, step=${current.conversationStep}, " +
                "pendingAutoMsg='${current.pendingAutoMessage.take(30)}', " +
        ...
        if (current.pendingAutoMessage.isNotEmpty()) {
            logDebug("JustACalc", "!! ++ BLOCKED by pendingAutoMessage: '${current.pendingAutoMessage.take(50)}'")
            return
        }
        ...
        if (current.conversationStep in AUTO_PROGRESS_STEPS) {
            logDebug("JustACalc", "!! ${if (accepted) "++" else "--"} BLOCKED by AUTO_PROGRESS_STEPS at step ${current.conversationStep}")
            return
        }
        ...
        if (isPermissionStep && current.isTyping) {
            logDebug("JustACalc", "!! ${if (accepted) "++" else "--"} BLOCKED by isTyping at permission-trigger step ${current.conversationStep}")
            return
        }
        ...
        val timeoutUntil = if (!accepted && stepConfig.timeoutMinutes > 0) {
            nowMillis() + (stepConfig.timeoutMinutes * 60 * 1000)
        } else {
            0L
        }
